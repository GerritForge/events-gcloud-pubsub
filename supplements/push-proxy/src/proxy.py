...
def index():
    ...
    enduser_endpoint = f"https://{host}{request.args.get('path')}"
    enduser_endpoint_with_token = (
        f"{enduser_endpoint}?token={request.args.get('token')}"
    )
    logging.debug(f"Forwarding to host {enduser_endpoint_with_token}")

    session = requests.Session()

    headers = {"Authorization": _get_auth_header(request, enduser_endpoint)}
