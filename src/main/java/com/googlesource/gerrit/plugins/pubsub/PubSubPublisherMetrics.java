...
public class PubSubPublisherMetrics extends PubSubEventsMetrics {
  ...
  @Inject
  public PubSubPublisherMetrics(MetricMaker metricMaker) {

    this.publisherSuccessCounter =
        metricMaker.newCounter(
            "gcloud_pubsub/broker/broker_message_publisher_counter",
            new Description("Number of successfully published messages")
                .setRate()
                .setUnit("messages"),
            stringField(PUBLISHER_SUCCESS_COUNTER, "Count of published messages"));
    this.publisherFailureCounter =
        metricMaker.newCounter(
            "gcloud_pubsub/broker/broker_message_publisher_failure_counter",
            new Description("Number of messages failed to publish").setRate().setUnit("errors"),
            stringField(PUBLISHER_FAILURE_COUNTER, "Count of messages failed to publish"));
  }

  public void incrementSuccessPublishMessage() {
